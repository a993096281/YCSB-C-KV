...
cmd="./ycsbc -db nvmnvtree -dbpath $dbpath -threads 1 -P $workload -load true -morerun $moreworkloads -dbstatistics true >>out.out 2>&1 "

if [ "$1" == "numa" ];then
    cmd="numactl -N 1 -m 1 ./ycsbc -db nvmnvtree -dbpath $dbpath -threads 1 -P $workload -load true -morerun $moreworkloads -dbstatistics true >>out.out 2>&1 "
fi
